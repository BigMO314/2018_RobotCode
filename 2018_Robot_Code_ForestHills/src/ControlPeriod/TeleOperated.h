...
namespace ControlPeriod{
	...
	public:
		//Constructor
		TeleOperated(RobotMechanism::Drivetrain *rbt_Drivetrain,RobotMechanism::Arm *rbt_Arm, RobotMechanism::Climber *rbt_Climber,MOLib::XboxController *ctl_Driver,MOLib::XboxController *ctl_Operator){
		...
		void Update(){
//			if (Dashboard.Misc.TestingMode.Get()) {
//				if (ctl_Driver->GetYButton()) rbt_Drivetrain->GoToDistance(350);
////			}
//
//			else {
				...
				if(ctl_Operator->GetBumper(XboxController::kLeftHand)){
					...
					else if(ctl_Operator->GetBackButton()) rbt_Arm->Point(rbt_Arm->PointPosition::kForwardPortal);
				}
					...
					else if(ctl_Operator->GetBackButton()) rbt_Arm->Point(rbt_Arm->PointPosition::kReversePortal);
				}
				else{
...
//			}
	...
	private:
		...
		RobotMechanism::Climber			*rbt_Climber;
